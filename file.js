...
Source code walkthrough video coming soon. Check back tomorrow.
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  Math.closestItCanBe = (p1, p2) => {
    let angle = Math.angle(p1, p2);
  ...
  Math.rollAroundCircle = (circle, ball) => {
    let impactAngle = Math.angle(ball, circle);
    let forceAngle = Math.angle(
      { x: 0, y: 0 },
      { x: ball.velocityX, y: ball.velocityY }
    );
    let impactForceAngle = impactAngle - forceAngle;

    const forceDistance = Math.distance2D(
      { x: 0, y: 0 },
      { x: ball.velocityX, y: ball.velocityY }
    );
    const lengthOfSideways = Math.sin(impactForceAngle) * forceDistance;

    const ballDistance = wallW / 2 + ballSize / 2;
    const rotationAngle = Math.atan(lengthOfSideways / ballDistance);

    const delta = {
      x: Math.cos(impactAngle + Math.PI - rotationAngle) * ballDistance,
      y: Math.sin(impactAngle + Math.PI - rotationAngle) * ballDistance
    };

    const x = ball.x;
    const y = ball.y;
    const velocityX = ball.x - (delta.x + circle.x);
    const velocityY = ball.y - (delta.y + circle.y);
  ...
  const walls = [
    ...
    { column: 3, row: 8, horizontal: true, length: 4 },
  ...
  window.addEventListener("keydown", function (event) {
    ...
    if (event.key == "H" || event.key == "h") {
      ...
      noteElement.innerHTML = `
        Hard mode with black holes coming soon.<br/>Check back tomorrow.
        <p>
          More Info about me
          <a href="https://v4jdzfa9hzjqmnlh6fsxoq-on.drv.tw/www.mywebsite/Rohith/" , target="_blank">Just Rohith</a>
        </p>`;
  ...
  function resetGame() {
    ...
    if (ballElements.length) {
      balls.forEach(({ x, y }, index) => {
        ballElements[index].ball.style.cssText = `left: ${x}px; top: ${y}px; `;
      });
    }
  }
  ...
  function main(timestamp) {
    ...
    // Should be 16 milliseconds on an average
    const timeElapsed = timestamp - previousTimestamp;
    const progress = timeElapsed / 16; // Should = 1 on average

    // If mouse didn't move yet don't do anything
    if (accelerationX != undefined && accelerationY != undefined) {
      const velocityChangeX = accelerationX * progress;
      const velocityChangeY = accelerationY * progress;
      const frictionDeltaX = frictionX * progress;
      const frictionDeltaY = frictionY * progress;

      balls.forEach((ball) => {
        if (velocityChangeX == 0) {
          // No rotation, the plane is flat
          // On flat surface friction can only slow down, but not reverse movement
          ball.velocityX = Math.slow(ball.velocityX, frictionDeltaX);
        } else {
        ...
        if (velocityChangeY == 0) {
          // No rotation, the plane is flat
          // On flat surface friction can only slow down, but not reverse movement
          ball.velocityY = Math.slow(ball.velocityY, frictionDeltaY);
        } else {
          ...
          if (wall.horizontal) {
            ...
            if (
              ...
              if (
                ...
                const distance = Math.distance2D(wallStart, {
                  x: ball.nextX,
                  y: ball.nextY
                });
                if (distance < ballSize / 2 + wallW / 2) {
                  ...
                  const closest = Math.closestItCanBe(wallStart, {
                    x: ball.nextX,
                    y: ball.nextY
                  });
                  const rolled = Math.rollAroundCircle(wallStart, {
                    x: closest.x,
                    y: closest.y,
              ...
              if (
                ...
                const distance = Math.distance2D(wallEnd, {
                  x: ball.nextX,
                  y: ball.nextY
                });
                if (distance < ballSize / 2 + wallW / 2) {
                  ...
                  const closest = Math.closestItCanBe(wallEnd, {
                    x: ball.nextX,
                    y: ball.nextY
                  });
                  const rolled = Math.rollAroundCircle(wallEnd, {
                    x: closest.x,
                    y: closest.y,
                    velocityX: ball.velocityX,
                    velocityY: ball.velocityY
    ...
    if (
      balls.every(
        (ball) => Math.distance2D(ball, { x: 350 / 2, y: 315 / 2 }) < 65 / 2
      )
    ) {
